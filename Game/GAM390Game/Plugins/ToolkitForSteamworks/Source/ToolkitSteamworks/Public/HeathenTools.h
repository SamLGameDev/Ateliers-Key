...
#define STEAM_CLIENT(returnValue) \
    ...
    if (!SteamToolsSubsystem || !SteamUser()) \
    { \
        UE_LOG(LogTemp, Warning, TEXT("Steam API is not initialized or SteamToolsSubsystem is not available")); \
        return returnValue; \
    }
...
#define STEAM_SERVER(returnValue) \
    USteamToolsSubsystem* SteamToolsSubsystem = USteamToolsSubsystem::GetSteamToolsSubsystem(); \
    if (!SteamToolsSubsystem || !SteamGameServer()) \
    { \
        UE_LOG(LogTemp, Warning, TEXT("Steam API is not initialized or SteamToolsSubsystem is not available")); \
        return returnValue; \
    }
...
enum class UEDeviceFormFactor : uint8
{
    EPC_Unknown UMETA(DisplayName = "Unknown"),
    EPC_Phone UMETA(DisplayName = "Phone"),
    EPC_Tablet UMETA(DisplayName = "Tablet"),
    EPC_Computer UMETA(DisplayName = "Computer"),
    EPC_TV UMETA(DisplayName = "TV")
};
...
UENUM(BlueprintType, Category = "Heathen's Toolkit|Steamworks")
enum class UEGamepadTextInputLineMode : uint8
{
    EPC_GamepadTextInputLineModeSingleLine = 0 UMETA(DisplayName = "Single Line"),
    EPC_GamepadTextInputLineModeMultipleLines = 1 UMETA(DisplayName = "Multiple Lines"),
};

UENUM(BlueprintType, Category = "Heathen's Toolkit|Steamworks")
enum class UEGamepadTextInputMode : uint8
{
    EPC_GamepadTextInputModeNormal = 0 UMETA(DisplayName = "Normal"),
    EPC_GamepadTextInputModePassword = 1 UMETA(DisplayName = "Password"),
};

UENUM(BlueprintType, Category = "Heathen's Toolkit|Steamworks")
enum class UEFloatingGamepadTextInputMode : uint8
{
    EPC_FloatingGamepadTextInputModeModeSingleLine = 0 UMETA(DisplayName = "Single Line"),
    EPC_FloatingGamepadTextInputModeModeMultipleLines = 1 UMETA(DisplayName = "Multiple Lines"),
    EPC_FloatingGamepadTextInputModeModeEmail = 2 UMETA(DisplayName = "Email"),
    EPC_FloatingGamepadTextInputModeModeNumeric = 3 UMETA(DisplayName = "Numeric"),
};
...
enum class UEResult : uint8
{
    ...
    EPC_AccountNotFeatured = 45 UMETA(DisplayName = "Account Not Featured"),
...
public:
    ...
    TArray<uint8> Ticket;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Heathen's Toolkit|Steamworks")
    UEBeginAuthSessionResult RequestState;
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Heathen's Toolkit|Steamworks")
    UEAuthSessionResponse RequestResponse;
};
...
USTRUCT(BlueprintType)
struct FItemTypeCount
{
    GENERATED_BODY()

public:
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Heathen's Toolkit|Steamworks")
    int64 DefinitionId = 0;
...
* Delegates
******************************************/
DECLARE_DYNAMIC_DELEGATE_ThreeParams(FBeginAuthSessionCallbackDelegate, int64, User, int64, Owner, UEAuthSessionResponse, Response);
DECLARE_DYNAMIC_DELEGATE_ThreeParams(FWebApiResponseCallbackDelegate, UEResult, result, int32, authTicketHandle, const TArray<uint8>&, ticket);
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FSteamRemotePlaySessionConnectedCallback, int32, SessionID);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FSteamRemotePlaySessionDisconnectedCallback, int32, SessionID);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FSteamRemotePlayTogetherGuestInviteCallback, FString, ConnectURL);
