...
bool UToolkitSteamworksBPLibrary::IsSteamRunningOnSteamDeck()
{
	STEAM_CLIENT(false);
#if STEAM_MAJOR > 1 || STEAM_MINOR > 52
	return SteamUtils()->IsSteamRunningOnSteamDeck();
#else
	UE_LOG(LogTemp, Warning, TEXT("IsSteamRunningOnSteamDeck is not supported in this version of Steamworks SDK (version 1.51 or earlier)."));
	return false;
#endif
}

bool UToolkitSteamworksBPLibrary::IsSteamRunningInVR()
{
	STEAM_CLIENT(false);
...
int64 UToolkitSteamworksBPLibrary::SteamAppIdToGameId(int32 AppId)
{
    ...
    return static_cast<int64>(game.ToUint64());
}
...
FDLCData UToolkitSteamworksBPLibrary::SteamGetDLCDataByIndex(int32 Index)
{
    ...
    if (SteamApps()->BGetDLCDataByIndex(Index, &appId, &available, name, 128))
    {
        ...
        result.Name = FString(FUTF8ToTCHAR(name));
    }
...
FString UToolkitSteamworksBPLibrary::SteamGetCurrentBetaName()
{
	...
	return FString(FUTF8ToTCHAR(buffer));
}

FString UToolkitSteamworksBPLibrary::SteamGetCurrentGameLanguage()
{
	STEAM_CLIENT(FString());
	return FString(FUTF8ToTCHAR(SteamApps()->GetCurrentGameLanguage()));
}
...
FString UToolkitSteamworksBPLibrary::SteamGetLaunchCommandLine()
{
	...
	if (count > 0)
		return FString(FUTF8ToTCHAR(commandLine));
	else
		return FString();
}
...
FString UToolkitSteamworksBPLibrary::SteamGetLaunchQueryParam(FString key)
{
	...
	return FString(FUTF8ToTCHAR(value));
}
...
{
	STEAM_SERVER(;);
#if STEAM_MAJOR > 1 || STEAM_MINOR > 51
...
{
	STEAM_CLIENT(FString());
	return FString(FUTF8ToTCHAR(SteamFriends()->GetPersonaName()));
}
...
UEPersonaState UToolkitSteamworksBPLibrary::SteamGetFriendPersonaState(int64 SteamID)
{
	STEAM_CLIENT(UEPersonaState::EPC_Offline);
	return static_cast<UEPersonaState>(SteamFriends()->GetFriendPersonaState(INT64_TO_STEAMID(SteamID)));
}
...
FString UToolkitSteamworksBPLibrary::SteamGetFriendGroupName(int32 friendGroupId)
{
	...
	if (GroupNameBuffer)
	{
		FString GroupName = FString(FUTF8ToTCHAR(GroupNameBuffer));
		return GroupName;
	}
	else
		return FString();
...
FString UToolkitSteamworksBPLibrary::SteamGetConnectCommandLineValue()
{
...
{
	STEAM_CLIENT(FString());
	const char* RawName = SteamFriends()->GetFriendPersonaName(INT64_TO_STEAMID(id));
	return FString(FUTF8ToTCHAR(RawName));
}

FString UToolkitSteamworksBPLibrary::SteamGetPlayerNickname(int64 id)
{
	STEAM_CLIENT(FString());
	const char* RawName = SteamFriends()->GetPlayerNickname(INT64_TO_STEAMID(id));
	return FString(FUTF8ToTCHAR(RawName));
}
...
void UToolkitSteamworksBPLibrary::SteamGetMySteamAvatar(FAvatarLoadCallback Callback)
{
	...
	SteamToolsSubsystem->GetAvatarBySteamId(SteamUser()->GetSteamID(), Callback);
}
...
{
	STEAM_CLIENT_INSTANCE(;);
	SteamToolsSubsystem->GetAvatarBySteamId(INT64_TO_STEAMID(id), Callback);
}
...
void UToolkitSteamworksBPLibrary::SteamActivateGameOverlayRemotePlayTogetherInviteDialog(int64 lobbyId)
{
	STEAM_CLIENT_INSTANCE(;);
	SteamFriends()->ActivateGameOverlayRemotePlayTogetherInviteDialog(INT64_TO_STEAMID(lobbyId));
}

void UToolkitSteamworksBPLibrary::SteamActivateGameOverlayInviteStringDialog(FString ConnectionString)
{
	STEAM_CLIENT_INSTANCE(;);
...
{
	STEAM_CLIENT_INSTANCE(;);
	if (dialog == UEOverlayUserDialog::EPC_SteamId)
		SteamFriends()->ActivateGameOverlayToUser("steamid", INT64_TO_STEAMID(userId));
	else if (dialog == UEOverlayUserDialog::EPC_Chat)
		SteamFriends()->ActivateGameOverlayToUser("chat", INT64_TO_STEAMID(userId));
	else if (dialog == UEOverlayUserDialog::EPC_JoinTrade)
		SteamFriends()->ActivateGameOverlayToUser("jointrade", INT64_TO_STEAMID(userId));
	else if (dialog == UEOverlayUserDialog::EPC_Stats)
		SteamFriends()->ActivateGameOverlayToUser("stats", INT64_TO_STEAMID(userId));
	else if (dialog == UEOverlayUserDialog::EPC_Achievements)
		SteamFriends()->ActivateGameOverlayToUser("achievements", INT64_TO_STEAMID(userId));
	else if (dialog == UEOverlayUserDialog::EPC_FriendAdd)
		SteamFriends()->ActivateGameOverlayToUser("friendadd", INT64_TO_STEAMID(userId));
	else if (dialog == UEOverlayUserDialog::EPC_FriendRemove)
		SteamFriends()->ActivateGameOverlayToUser("friendremove", INT64_TO_STEAMID(userId));
	else if (dialog == UEOverlayUserDialog::EPC_FriendRequestAccept)
		SteamFriends()->ActivateGameOverlayToUser("friendrequestaccept", INT64_TO_STEAMID(userId));
	else if (dialog == UEOverlayUserDialog::EPC_FriendRequestIgnore)
		SteamFriends()->ActivateGameOverlayToUser("friendrequestignore", INT64_TO_STEAMID(userId));
}
...
bool UToolkitSteamworksBPLibrary::SteamSimpleAddPromoItem(int32 itemDef, TArray<FString> readProperties, FItemResultsCallback callback)
{
	...
	if (response)
	{
		SteamToolsSubsystem->InventoryRequests.Add(handle, request);
	}
	return response;
}
...
bool UToolkitSteamworksBPLibrary::SteamSimpleExchangeItems(int32 itemDef, TArray<FItemCount> recipe, TArray<FString> readProperties, FItemResultsCallback callback)
{
	...
	generateItems.Add(SteamItemDef_t(itemDef));
...
bool UToolkitSteamworksBPLibrary::SteamSimpleGenerateItems(TArray<FItemTypeCount> items, TArray<FString> readProperties, FItemResultsCallback callback)
{
	STEAM_CLIENT_INSTANCE(false);
	FItemResultRequest request = FItemResultRequest();
	request.properties = readProperties;
	...
	{
		itemDefs.Add(static_cast<SteamItemDef_t>(item.DefinitionId));
...
bool UToolkitSteamworksBPLibrary::SteamSimpleTransferItemQuantity(int64 sourceInstanceId, int32 quantity, int64 destinationInstanceId, TArray<FString> readProperties, FItemResultsCallback callback)
{
	...
	if (response)
	{
		SteamToolsSubsystem->InventoryRequests.Add(result, request);
	}
	return response;
}
...
FInventoryResult UToolkitSteamworksBPLibrary::SteamGenerateItems(TArray<FItemTypeCount> items)
{
	FInventoryResult result = FInventoryResult();
	STEAM_CLIENT(result);
...
TArray<FString> UToolkitSteamworksBPLibrary::SteamGetItemDefinitionProperties(int32 itemDef)
{
	...
	{
		ItemProperty = FString(FUTF8ToTCHAR(pchValueBuffer));
				...
				for (const FString& prop : properties)
				{
					...
					if (SteamInventory()->GetResultItemProperty(resultHandle, i, StringCast<ANSICHAR>(*prop).Get(), NULL, &bufferSize))
					{
						...
						if (SteamInventory()->GetResultItemProperty(resultHandle, i, StringCast<ANSICHAR>(*prop).Get(), pchValueBuffer, &bufferSize))
						{
							...
							pair.Key = prop;
							pair.Value = FString(FUTF8ToTCHAR(pchValueBuffer));
...
void UToolkitSteamworksBPLibrary::SteamGetItemCount(UInventoryItemDataAsset* Item, int32 DesiredCount, TArray<FItemCount>& ItemCounts, int32& TotalFound, bool& Success)
{
	TotalFound = 0;
	Success = false;

	if (!Item) return; // Early exit if Item is null

	for (const FItemDetailWithProperties& Detail : Item->ItemDetailsArray)
	{
		if (TotalFound >= DesiredCount) break; // Stop once we meet the desired count

		int32 AmountToTake = FMath::Min(DesiredCount - TotalFound, Detail.Quantity);
		if (AmountToTake > 0)
		{
			FItemCount NewItemCount;
			NewItemCount.InstanceId = Detail.InstanceId;
			NewItemCount.Quantity = AmountToTake;

			ItemCounts.Add(NewItemCount);
			TotalFound += AmountToTake;
		}
	}

	Success = (TotalFound == DesiredCount);
}
...
{
	STEAM_CLIENT(FString());
	return FString(FUTF8ToTCHAR(SteamMatchmaking()->GetLobbyData(INT64_TO_STEAMID(lobbyId), StringCast<ANSICHAR>(*key).Get())));
}
...
FLobbyData UToolkitSteamworksBPLibrary::SteamGetLobbyDataByIndex(int64 lobbyId, int32 index)
{
	...
	if (response)
	{
		result.Key = FString(FUTF8ToTCHAR(keyBuffer));
		result.Value = FString(FUTF8ToTCHAR(valueBuffer));
	}
	return result;
}
...
{
	STEAM_CLIENT(FString());
	return FString(FUTF8ToTCHAR(SteamMatchmaking()->GetLobbyMemberData(INT64_TO_STEAMID(lobbyId), INT64_TO_STEAMID(userId), StringCast<ANSICHAR>(*key).Get())));
}
...
int64 UToolkitSteamworksBPLibrary::SteamGetConnectLobbyCommandLineValue()
{ 
	FString FullCommandLine = FCommandLine::Get();
	int64 LobbyID = 0;

	FString SearchKey = TEXT("+connect_lobby");
	int32 Index = FullCommandLine.Find(SearchKey);
	if (Index != INDEX_NONE)
	{
		int32 Start = Index + SearchKey.Len();
		FString SubStr = FullCommandLine.Mid(Start).TrimStart();

		int32 End = 0;
		while (End < SubStr.Len() && FChar::IsDigit(SubStr[End]))
		{
			End++;
		}

		FString LobbyIDStr = SubStr.Left(End);
		LobbyID = FCString::Strtoi64(*LobbyIDStr, nullptr, 10);

		UE_LOG(LogTemp, Log, TEXT("Lobby Steam ID found via manual parse: %lld"), LobbyID);
	}
	else
	{
...
{
	STEAM_CLIENT(;);
	SteamToolsSubsystem->MemberOfLobbies.Remove(lobbyId);
...
TArray<uint8> UToolkitSteamworksBPLibrary::SteamUGCRead(int64 ugcHandle, int size)
{
	...
	int dataRead = SteamRemoteStorage()->UGCRead(static_cast<UGCHandle_t>(ugcHandle), fileData.GetData(), size, 0, EUGCReadAction::k_EUGCRead_ContinueReadingUntilFinished);
...
#else
	UE_LOG(LogTemp, Warning, TEXT("SteamBeginFileWriteBatch is not supported in this version of Steamworks SDK (version 1.51 or earlier)."));
	return false;
#endif
}
...
#else
	UE_LOG(LogTemp, Warning, TEXT("SteamEndFileWriteBatch is not supported in this version of Steamworks SDK (version 1.51 or earlier)."));
	return false;
#endif
}
...
int UToolkitSteamworksBPLibrary::SteamGetFileCount()
{
	STEAM_CLIENT(false);
...
FRemoteStorageFileInfo UToolkitSteamworksBPLibrary::SteamGetFileNameAndSize(int32 index)
{
	...
	STEAM_CLIENT(result);
	result.FileName = FString(FUTF8ToTCHAR(SteamRemoteStorage()->GetFileNameAndSize(index, &(result.FileSize))));
	return result;
}
...
FRemoteStorageUGCDetails UToolkitSteamworksBPLibrary::SteamGetUGCDetails(int64 ugcHandle)
{
	...
	if (SteamRemoteStorage()->GetUGCDetails(static_cast<UGCHandle_t>(ugcHandle), &appId, &name, &fileLength, &owner))
	{
		...
		result.AppId = appId;
		result.FileName = FString(FUTF8ToTCHAR(name));
...
FSteamInstallationInfo UToolkitSteamworksBPLibrary::SteamGetItemInstallInfo(int64 fileId)
{
	...
	result.Path = FString(FUTF8ToTCHAR(folder));
...
FUGCAdditionalPreviewResult UToolkitSteamworksBPLibrary::SteamGetQueryUGCAdditionalPreview(int64 queryHandle, int32 index, int32 previewIndex)
{
	...
	result.OriginalFileName = FString(FUTF8ToTCHAR(originalfileName));
	result.UrlOrVideoId = FString(FUTF8ToTCHAR(videoId));
...
FString UToolkitSteamworksBPLibrary::SteamGetQueryUGCTag(int64 queryHandle, int32 index, int32 tagIndex)
{
	...
	FString result = FString(FUTF8ToTCHAR(value));
	return result;
}
...
FKeyValuePair UToolkitSteamworksBPLibrary::SteamGetQueryUGCKeyValueTag(int64 queryHandle, int32 index, int32 tagIndex)
{
	...
	result.Key = FString(FUTF8ToTCHAR(key));
	result.Value = FString(FUTF8ToTCHAR(value));
...
FSteamUGCDetails UToolkitSteamworksBPLibrary::SteamGetQueryUGCResult(int64 queryHandle, int32 index)
{
	...
	if (SteamUGC()->GetQueryUGCResult(static_cast<UGCQueryHandle_t>(queryHandle), static_cast<uint32>(index), &details))
	{
		...
		result.Description = FString(FUTF8ToTCHAR(details.m_rgchDescription));
		result.FileHandle = static_cast<int64>(details.m_hFile);
		result.FileId = static_cast<int64>(details.m_nPublishedFileId);
		result.FileName = FString(FUTF8ToTCHAR(details.m_pchFileName));
		...
		FString tagString = FString(FUTF8ToTCHAR(details.m_rgchTags));
		...
		result.Title = FString(FUTF8ToTCHAR(details.m_rgchTitle));
		result.Url = FString(FUTF8ToTCHAR(details.m_rgchURL));
...
#else
	UE_LOG(LogTemp, Warning, TEXT("SteamShowWorkshopEULA is not supported in this version of Steamworks SDK (version 1.51 or earlier)."));
	return false;
#endif
}

bool UToolkitSteamworksBPLibrary::SteamShowGamepadTextInput(UEGamepadTextInputMode InputMode, UEGamepadTextInputLineMode LineMode, FString Description, int32 MaxChar, FString ExistingText)
{
	STEAM_CLIENT(false);
#if STEAM_MAJOR > 1 || STEAM_MINOR > 51
	return SteamUtils()->ShowGamepadTextInput(static_cast<EGamepadTextInputMode>(InputMode), static_cast<EGamepadTextInputLineMode>(LineMode), StringCast<ANSICHAR>(*Description).Get(), MaxChar, StringCast<ANSICHAR>(*ExistingText).Get());
#else
	UE_LOG(LogTemp, Warning, TEXT("SteamShowGamepadTextInput is not supported in this version of Steamworks SDK (version 1.51 or earlier)."));
	return false;
#endif
}

bool UToolkitSteamworksBPLibrary::SteamShowFloatingGamepadTextInput(UEFloatingGamepadTextInputMode KeyboardMode, int32 PositionX, int32 PositionY, int32 Width, int32 Height)
{
	STEAM_CLIENT(false);
#if STEAM_MAJOR > 1 || STEAM_MINOR > 51
	return SteamUtils()->ShowFloatingGamepadTextInput(static_cast<EFloatingGamepadTextInputMode>(KeyboardMode), PositionX, PositionY, Width, Height);
#else
	UE_LOG(LogTemp, Warning, TEXT("SteamShowFloatingGamepadTextInput is not supported in this version of Steamworks SDK (version 1.51 or earlier)."));
	return false;
#endif
}

void UToolkitSteamworksBPLibrary::SteamStartVrDashboard()
{
	STEAM_CLIENT(;);
...
#else
	UE_LOG(LogTemp, Warning, TEXT("SteamGetWorkshopEULAStatus is not supported in this version of Steamworks SDK (version 1.51 or earlier)."));
#endif
}
...
int32 UToolkitSteamworksBPLibrary::SteamGetFriendSteamLevel(int64 SteamID)
{
	STEAM_CLIENT(0);
	return SteamFriends()->GetFriendSteamLevel(INT64_TO_STEAMID(SteamID));
}
...
FDecompressVoiceResult UToolkitSteamworksBPLibrary::SteamDecompressVoice(TArray<uint8> CompressedData, int32 DesiredSampleRate, USoundWaveProcedural* buffer)
{
	...
	{
		Result.Result = UEVoiceResult::EPC_NoData;
		Result.bSuccess = false;
		return Result;
	}

	// Use the static 8KB buffer from  our Steam Game Instance ... this aligns better with Valve's recommended practice
	uint8* DestBuffer = USteamToolsSubsystem::VoiceDataBuffer;
	int32 DestBufferSize = sizeof(USteamToolsSubsystem::VoiceDataBuffer);
	...
	if (DecompressResult == k_EVoiceResultOK && BytesWritten > 0)
	{
		// Resize the DecompressedVoiceData array to fit the decompressed data and copy to it
		Result.DecompressedVoiceData.SetNum(BytesWritten);
		FMemory::Memcpy(Result.DecompressedVoiceData.GetData(), DestBuffer, BytesWritten);

		Result.Result = static_cast<UEVoiceResult>(DecompressResult);
		Result.bSuccess = true;

		// Create or use the provided UProceduralSoundWave instance
		if (!buffer)
		{
	...
	{
		Result.DecompressedVoiceData.Empty();
		Result.Result = static_cast<UEVoiceResult>(DecompressResult);
		Result.bSuccess = false;
	}
...
UEBeginAuthSessionResult UToolkitSteamworksBPLibrary::SteamBeginAuthSession(TArray<uint8> ticket, int64 steamId)
{
	...
	// Call the actual BeginAuthSession function
	EBeginAuthSessionResult result = SteamUser()->BeginAuthSession(TicketData, TicketLength, INT64_TO_STEAMID(steamId));
...
FVoiceResult UToolkitSteamworksBPLibrary::SteamGetVoice()
{
	...
	if (result != k_EVoiceResultOK || availableData == 0)
		return defaultVal;

	defaultVal.VoiceData.SetNum(availableData);
	uint32 bytesWritten = 0;
	result = SteamUser()->GetVoice(true, defaultVal.VoiceData.GetData(), availableData, &bytesWritten);

	if (result == k_EVoiceResultOK && bytesWritten > 0)
	{
		defaultVal.VoiceData.SetNum(bytesWritten); 
		defaultVal.Result = static_cast<UEVoiceResult>(result);
	}
	else
	{
		defaultVal.VoiceData.Empty(); 
		defaultVal.Result = static_cast<UEVoiceResult>(result);
	}
...
FString UToolkitSteamworksBPLibrary::SteamGetRichPresence(FString key)
{
	...
	return FString(FUTF8ToTCHAR(result));
}
...
FAchievementStatus UToolkitSteamworksBPLibrary::SteamGetAchievement(FString apiName)
{
	...
	SteamUserStats()->GetAchievementAchievedPercent(Name, &pert);

	FString name = FString(SteamUserStats()->GetAchievementDisplayAttribute(Name, "name"));
	FString desc = FString(SteamUserStats()->GetAchievementDisplayAttribute(Name, "desc"));

	const char* DisplayAttribute = SteamUserStats()->GetAchievementDisplayAttribute(Name, "hidden");
...
{
	STEAM_CLIENT(FString());
	return FString(FUTF8ToTCHAR(SteamUserStats()->GetAchievementName(index)));
}
...
FString UToolkitSteamworksBPLibrary::SteamGetLeaderboardName(int64 boardId)
{
	STEAM_CLIENT(FString());
	SteamLeaderboard_t id = static_cast<SteamLeaderboard_t>(boardId);
	return FString(FUTF8ToTCHAR(SteamUserStats()->GetLeaderboardName(id)));
}
...
int UToolkitSteamworksBPLibrary::SteamGetRemotePlaySessionCount()
{
	STEAM_CLIENT(0);
	return SteamRemotePlay()->GetSessionCount();
}

int UToolkitSteamworksBPLibrary::SteamGetRemotePlaySessionID(int Index)
{
	STEAM_CLIENT(0);
	uint32 unSessionID = SteamRemotePlay()->GetSessionID(Index);
	return static_cast<int>(unSessionID);
}

int64 UToolkitSteamworksBPLibrary::SteamGetRemotePlaySessionUserID(int SessionID)
{
	STEAM_CLIENT(0);
	RemotePlaySessionID_t unSessionID = RemotePlaySessionID_t(static_cast<uint32>(SessionID));
	CSteamID unUserID = SteamRemotePlay()->GetSessionSteamID(unSessionID);
	return STEAMID_TO_INT64(unUserID);
}

FString UToolkitSteamworksBPLibrary::SteamGetRemotePlayClientName(int SessionID)
{
	STEAM_CLIENT(FString());
	RemotePlaySessionID_t unSessionID = RemotePlaySessionID_t(static_cast<uint32>(SessionID));
	const char* result = SteamRemotePlay()->GetSessionClientName(unSessionID);
	return FString(FUTF8ToTCHAR(result));
}

UEDeviceFormFactor UToolkitSteamworksBPLibrary::SteamGetRemotePlayClientFormFactor(int SessionID)
{
	STEAM_CLIENT(UEDeviceFormFactor::EPC_Unknown);
	RemotePlaySessionID_t unSessionID = RemotePlaySessionID_t(static_cast<uint32>(SessionID));
	ESteamDeviceFormFactor EForm = SteamRemotePlay()->GetSessionClientFormFactor(unSessionID);
	return static_cast<UEDeviceFormFactor>(EForm);
}

bool UToolkitSteamworksBPLibrary::SteamGetSessionClientResolution(int SessionID, UPARAM(ref)int32& Width, UPARAM(ref)int32& Height)
{
	STEAM_CLIENT(false);
	RemotePlaySessionID_t unSessionID = RemotePlaySessionID_t(static_cast<uint32>(SessionID));	
	return SteamRemotePlay()->BGetSessionClientResolution(unSessionID, &Width, &Height);
}

bool UToolkitSteamworksBPLibrary::SteamSendRemotePlayTogetherInvite(int64 SteamID)
{
	STEAM_CLIENT(false);
	return SteamRemotePlay()->BSendRemotePlayTogetherInvite(INT64_TO_STEAMID(SteamID));
}
